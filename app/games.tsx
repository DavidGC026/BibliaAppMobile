...
import { ActivityIndicator, AppState, BackHandler, ScrollView, View } from 'react-native';
import { useAuth } from '@/context/AuthContext';
import { useAppTheme } from '@/hooks/useAppTheme';
import { useContentPadding } from '@/hooks/useContentPadding';
import { getMeta, setMeta } from '@/lib/db';
import { loadGameContent, synchronizeGameProgress } from '@/lib/api';
...
import { FreeOptions, SavedRounds, ProgressSync, WeeklyProgress } from '@/components/games/TrainingPanels';
...
function GamesHub({ userId, isAdmin }: { userId?: number; isAdmin: boolean }) {
  ...
  const session = useGamesSession({ storage, storageKey: `games-v2-${userId ?? 'guest'}`, legacyKey: `games-v1-${userId ?? 'guest'}`, loadContent: loadGameContent, userId, synchronize: synchronizeGameProgress });
  ...
  }, [active?.mountKey, view]);
  useEffect(() => {
    const listener = AppState.addEventListener('change', state => { if (state === 'active') { void session.sync(); void session.refresh(); } });
  ...
  const roundProps = { onComplete: session.onComplete, onOpen, onRestart: session.restart, settings: active?.settings, onAttempt: session.onAttempt, roundId: active?.id, checkpoint: active?.checkpoint, onCheckpoint: session.onCheckpoint };
  ...
  return <ScrollView ref={scroll} keyboardShouldPersistTaps="handled" style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingBottom: contentPadding, gap: 24, maxWidth: 720, width: '100%', alignSelf: 'center' }}>
    {view === 'editor' && isAdmin ? <><GameButton secondary label="Volver a juegos" onPress={() => { setView('free'); void session.refresh(); }} /><ContentEditor onEdit={() => scroll.current?.scrollTo({ y: 0, animated: false })} /></> : game && active ? <>
      ...
      {active.settings.level && <GameText muted>Nivel {LEVEL_LABELS[active.settings.level].toLowerCase()}</GameText>}
      <View key={active.mountKey}>
      ...
      {view === 'week' ? <WeeklyProgress session={session} /> : view !== 'review' ? GAME_CATALOG.map(entry => {
        ...
        return <GameCard key={entry.id}>
          ...
          <GameText muted>{result ? `Completado · ${result.score}/100` : entry.id === 'wordle' && view === 'free' ? `${session.availableWords} palabras · sin repetir con estos filtros` : entry.detail}</GameText>
          <GameButton label={`${result ? 'Repetir' : 'Jugar'} · ${entry.title}`} disabled={!session.ready || (view === 'daily' && !session.dailyAvailable) || (view === 'free' && entry.id === 'wordle' && !session.availableWords)} onPress={() => session.start(entry.id, view === 'daily' ? 'daily' : 'free')} />
        </GameCard>;
      ...
      <GameCard><GameText heading>Tus resultados</GameText><GameText muted>{totalPlayed} partidas · {totalPoints} puntos</GameText>
        ...
        <GameText muted>{userId ? 'Resultados, palabras jugadas, repasos y partidas sincronizados con tu cuenta en web y móvil cuando hay conexión.' : 'Resultados, palabras jugadas, repasos y partidas guardados en este dispositivo como visitante.'}</GameText>
      </GameCard>
    </>}
